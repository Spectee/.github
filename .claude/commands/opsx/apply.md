---
name: "OPSX: 実装"
description: OpenSpec変更からタスクを実装（実験的）
category: Workflow
...
**入力**: オプションで変更名を指定（例：`/opsx:apply add-auth`）。省略した場合は会話のコンテキストから推測を試みます。曖昧または不明確な場合は、利用可能な変更を表示して選択を促す必要があります。
...
3. **apply指示を取得**
   ...
   返される内容：
   - コンテキストファイルのパス（スキーマによって異なる）
   - 進捗（合計、完了、残り）
   - タスクリストと状態
   - 現在の状態に基づく動的な指示

   **状態の処理:**
   - `state: "blocked"`（アーティファクトが不足）の場合：メッセージを表示し、`/opsx:continue`の使用を提案
